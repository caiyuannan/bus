...
$(function() {
	...
	$.ajax({
		type: "POST",//提交的方式
		url: "workServlet?methodName=ajax2",//提交的地址
		data: "msg=" + str,//提交的数据
		dataType: "text",//希望返回的数据类型
		async: true,//异步操作
		success: function (msg) {//成功的方法  msg为返回数据
			var ob2 = JSON.parse(msg);
			console.log(ob2[0]['WDATE']);
			console.log(ob2[0]['STAFFID']);
			console.log(ob2[0]['SNAME']);
			$.each(ob2,function (i,n) {
				calendar.addEvent({
					id: n['STAFFID'],// 日程标题;
					title : n['SNAME'],// 日程标题
					start : new Date(n['WDATE']),// 日程起始时间
					end :  new Date(n['WDATE']),// 日程结束时间
					allDay : true
					// 日程是否全天
				});
			})
	...
	calendar = new FullCalendar.Calendar(calendarEl, {
		...
		eventClick : function(info) {
			...
			$("#gs2").text(info.event.title);
			$("#gs3").text(new Date(info.event.start).format('yyyy-MM-dd'));
				...
				buttons : [ {
					...
					click : function() {
						...
						if (personID!== null&& scheduleDate!=null) {
							var data ="del,"+personID+","+scheduleDate;
							//利用脚本的方式 创建 一个对象
							var ob ={sendMsg: data};
							//利用脚本的方式 创建 一个对象
		...
		buttons : [ {
			...
			click : function() {
				...
				if (personID!== null&& scheduleDate!=null) {
					...
					var data ="add,"+personID+","+scheduleDate;
					//利用脚本的方式 创建 一个对象
					var ob ={sendMsg: data};
					//对象转换成字符串
					var str = JSON.stringify(ob);
					$.ajax({
						...
						success: function (msg) {//成功的方法  msg为返回数据
							...
							if (ob2.sendMsg=="添加成功") {
								...
								alert("添加成功")
							}else if (ob2.sendMsg=="添加失败") {
								alert("添加失败")
							}else if (ob2.sendMsg=="该排班已经存在") {
								alert("该排班已经存在")
							}
